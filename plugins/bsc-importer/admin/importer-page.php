...
require_once plugin_dir_path( __FILE__ ) . 'reset-categories.php';
require_once plugin_dir_path( __FILE__ ) . 'seed-categories.php';
require_once plugin_dir_path( __FILE__ ) . 'reset-products.php';
require_once plugin_dir_path( __FILE__ ) . 'seed-products.php';

require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_Config.php';
require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_FileManager.php';
require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_MediaManager.php';
require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_MediaCleaner.php';
require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_ProductManager.php';
require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_Processor.php';
require_once plugin_dir_path( __FILE__ ) . 'classes/BSC_Theme_Importer_PPU_Init.php';

add_action( 'admin_post_bsc_theme_importer_download_categories', 'bsc_theme_importer_handle_download_categories' );
add_action( 'BSC_Theme_Importer_PPU_process_photos_event', array( 'BSC_Theme_Importer_PPU_Init', 'init' ) );
...
function bsc_theme_importer_plugin_asset_path( string $relative_path ): string {
	$base_path = defined( 'BSC_THEME_IMPORTER_PATH' ) ? BSC_THEME_IMPORTER_PATH : realpath( __DIR__ . '/..' );
...
function bsc_theme_importer_get_uploaded_file( string $field_name ): array {
    // phpcs:disable WordPress.Security.NonceVerification.Missing, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- Upload handlers call this only after validating the importer nonce.
...
function bsc_theme_importer_requires_confirmation( string $action ): bool {
	return in_array( $action, array( 'delete_categories', 'delete_products', 'process_product_photos' ), true );
}
...
function bsc_theme_importer_validate_action_confirmation( string $action ): void {
    ...
    // phpcs:disable WordPress.Security.NonceVerification.Missing -- Caller validates the importer action nonce before confirmation is checked.
...
function bsc_theme_importer_read_uploaded_json( string $field_name ): array {
	$file  = bsc_theme_importer_get_uploaded_file( $field_name );
...
function bsc_theme_importer_validate_product_photos_zip( ZipArchive $zip, int $compressed_size ): void {
	...
	$ignored_filenames     = array( '.ds_store', 'thumbs.db' );
...
function bsc_theme_importer_process_product_photos(): string {
	...
	[, $output] = bsc_theme_importer_capture_output(
...
function bsc_theme_importer_process_post_action(): array {
	...
	if ($request_method !== 'POST' || empty( $_POST['bsc_theme_importer_action'] )) {
		return array();
	}
	...
	if (
		...
		return array( bsc_theme_importer_make_notice( 'error', 'Solicitud no valida. Recarga la pagina e intenta de nuevo.' ) );
	}
	...
	try {
		...
		switch ($action) {
			case 'delete_categories':
				...
				[$deleted] = bsc_theme_importer_capture_output(
					static function (): int {
						return bsc_theme_importer_delete_categories();
					}
				);

				return array( bsc_theme_importer_make_notice( 'success', $deleted . ' categorias borradas.' ) );
			...
			case 'upload_categories':
				...
				[$created, $output] = bsc_theme_importer_capture_output(
			...
			case 'delete_products':
				...
				return array( bsc_theme_importer_make_notice( 'success', $deleted . ' productos borrados.' ) );
			...
			case 'upload_products':
				...
				[$processed, $output] = bsc_theme_importer_capture_output(
					static function () use ( $payload ) {
			...
			case 'upload_product_photos_zip':
				$size = bsc_theme_importer_upload_product_photos_zip( bsc_theme_importer_get_uploaded_file( 'product_photos_zip' ) );
				return array( bsc_theme_importer_make_notice( 'success', 'ZIP de imagenes cargado correctamente (' . $size . ').' ) );

			case 'process_product_photos':
				bsc_theme_importer_require_woocommerce();

				$details = bsc_theme_importer_process_product_photos();
				return array( bsc_theme_importer_make_notice( 'success', 'Procesamiento de imagenes finalizado.', $details ) );

			default:
				throw new RuntimeException( 'Accion no reconocida.' );
		}
	} catch (Throwable $throwable) {
		return array( bsc_theme_importer_make_notice( 'error', $throwable->getMessage() ) );
	}
}
...
function bsc_theme_importer_build_categories_export_data(): array {
	...
	if (is_wp_error( $product_categories )) {
        // phpcs:ignore WordPress.Security.EscapeOutput.ExceptionNotEscaped -- Exception messages are escaped by wp_die/render_notice before output.
	...
	$product_categories = array_values(
		array_filter(
			...
			static function ( $category ): bool {
				return !in_array( $category->slug, array( 'uncategorized', 'sin-categoria' ), true );
			}
		)
	);
...
function bsc_theme_importer_handle_download_categories(): void {
	...
	try {
        ...
        // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- JSON is generated by wp_json_encode and served as a download.
		echo $json;
		exit;
	} catch (Throwable $throwable) {
