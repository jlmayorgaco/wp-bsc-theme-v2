...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- CLI seeder sanity check.
$has_table = (int) $wpdb->get_var(
	$wpdb->prepare(
...
if ( $has_table === 0 ) {
	echo esc_html( "Ledger table {$ledger_table} does not exist. Aborting." ) . "\n";
...
function get_product_id_by_sku_safe( $sku ) {
	...
	global $wpdb;
  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- CLI seeder SKU lookup fallback.
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- CLI seeder cleanup for demo orders.
...
if ($order_ids) {
	foreach ($order_ids as $oid) {
	  ...
	  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- CLI seeder cleanup for demo ledger rows.
		$wpdb->delete( $ledger_table, array( 'order_id' => (int) $oid ), array( '%d' ) );
...
if ($demo_users) {
	...
	foreach ($demo_users as $u) {
	  ...
	  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- CLI seeder cleanup for demo ledger rows.
		$wpdb->delete( $ledger_table, array( 'user_id' => (int) $u->ID ), array( '%d' ) );
...
for ( $i = 1; $i <= $count_users; $i++ ) {
	...
	for ( $k = 1; $k <= $orders_per_user; $k++ ) {
		...
		if ($award_points > 0) {
			...
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- CLI seeder writes demo ledger data.
			$wpdb->insert(
				$ledger_table,
				array(
					'user_id'       => $user_id,
  ...
  // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery -- CLI seeder writes demo ledger data.
	$ok = $wpdb->insert(
		...
		array(
			...
			'reason'        => $reason,
			'order_id'      => null,
			'meta'          => wp_json_encode( array( 'note' => 'seed demo manual' ) ),
