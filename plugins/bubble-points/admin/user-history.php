...
function bsc_bp_render_user_history_screen($user_id) {
    ...
    if (!empty($_GET['error'])) {
        echo '<div class="notice notice-error is-dismissible"><p>'.esc_html(wp_kses_post(wp_unslash($_GET['error']))).'</p></div>';
    }
    ...
    echo '<p><strong>'.esc_html__('Current balance','bsc').':</strong> '.number_format_i18n($balance).'</p>';
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $row) {
            ...
            if ($meta_json !== '') {
                ...
                if (is_array($meta_arr)) {
                    ...
                    foreach ($meta_arr as $k => $v) {
                        if (is_scalar($v)) $extras[] = esc_html($k).': '.esc_html((string)$v);
                    }
                    if ($extras) {
            ...
            printf(
                ...
                esc_html($type_label),
                $order_cell, // already escaped/link-built
                $comments    // already escaped above
            );
        }
    }
