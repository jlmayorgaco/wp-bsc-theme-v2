...
function bsc_newsletter_export_csv(): void {
	...
	header( 'Content-Disposition: attachment; filename="newsletter-leads-' . wp_date( 'Y-m-d' ) . '.csv"' );
	...
	<div class="wrap bsc-admin-newsletter">
		<div class="bsc-admin-page-header bsc-admin-page-header--compact">
			<div>
				...
				<h1>Newsletter</h1>
				<p class="bsc-admin-page-header__description">Administra suscriptores, estados comerciales, notas internas y retencion de datos.</p>
			</div>
		...
		<form method="get" class="bsc-admin-newsletter__filters bsc-admin-filter-panel">
				...
				<tr>
					...
					<td>
						...
						<form method="post" class="bsc-admin-newsletter__delete-form">
							...
							<input type="hidden" name="subscriber_id" value="<?php echo esc_attr( $index ); ?>">
							<button type="submit" class="button button-small bsc-admin-button-danger" onclick="return confirm('Eliminar este lead y sus datos personales?');">Eliminar datos</button>
						</form>
					</td>
				</tr>
			<?php endforeach; ?>
			<?php if ( 0 === $visible_rows ) : ?>
				<tr><td colspan="7" class="bsc-admin-empty-state"><?php echo esc_html( ( $status_filter || $query_filter ) ? 'No hay leads con este filtro.' : 'No hay suscriptores todavia.' ); ?></td></tr>
			<?php endif; ?>
			</tbody>
		</table>
		</div>
