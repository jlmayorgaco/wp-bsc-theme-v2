...
function bsc_coupons_handle_actions(): void {
	...
	if ( isset( $_POST['bsc_create_coupon_nonce'] ) ) {
		...
		$code          = strtolower( sanitize_text_field( wp_unslash( $_POST['coupon_code'] ?? '' ) ) );
		$discount_type = sanitize_text_field( wp_unslash( $_POST['discount_type'] ?? 'percent' ) );
		$amount        = (float) sanitize_text_field( wp_unslash( $_POST['coupon_amount'] ?? 0 ) );
		...
		$min_amount    = (float) sanitize_text_field( wp_unslash( $_POST['minimum_amount'] ?? 0 ) );
	...
	<div class="wrap bsc-admin-coupons">
		...
		<?php if ( $bsc_notice ) : ?>
			...
			$notice_messages = array(
				'created'         => array( 'success', 'Cupón creado correctamente.' ),
				'deleted'         => array( 'success', 'Cupón eliminado.' ),
				'invalid_percent' => array( 'error', 'El porcentaje no puede ser mayor a 100%.' ),
				'invalid_expiry'  => array( 'error', 'La fecha de expiración no es válida.' ),
			);
			$notice_config   = $notice_messages[ $bsc_notice ] ?? array( 'success', 'Acción completada.' );
			?>
		...
		<table class="wp-list-table widefat fixed striped bsc-coupons-table">
			...
			<tr class="<?php echo esc_attr( $is_expired ? 'bsc-admin-coupons__expired' : '' ); ?>">
				<td>
					...
					<?php
					if ( $is_expired ) {
						echo '<br><span class="bsc-admin-coupons__expired-badge">Expirado</span>';}
					?>
				</td>
