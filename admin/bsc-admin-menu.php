...
function bsc_enqueue_dashboard_assets( string $hook ): void {
	...
	if ( ! in_array( $page, array( 'bsc-dashboard', 'bsc-settings' ), true ) ) {
		return;
	}
...
function bsc_dashboard_count_orders( array $statuses, array $extra_args = array() ): int {
...
function bsc_dashboard_get_range(): array {
	...
	if ( '7d' === $range_key ) {
		...
		$custom_end   = isset( $_GET['dashboard_end'] ) ? bsc_dashboard_parse_date_input( sanitize_text_field( wp_unslash( $_GET['dashboard_end'] ) ) ) : '';
...
function bsc_dashboard_empty_metrics(): array {
	return array(
		'counters'           => function_exists( 'bsc_metrics_empty_counters' ) ? bsc_metrics_empty_counters() : array(),
		'top_products'       => array(),
...
function bsc_render_dashboard(): void {
	...
	if ( 'POST' === $request_method && isset( $_POST['bsc_dashboard_action'] ) ) {
	...
	if ( false === $kpis ) {
		...
		if ( $can_view_financials ) {
			...
			bsc_reports_for_each_order(
				...
				function ( $o ) use ( &$range_revenue ) {
					if (in_array( $o->get_status(), array( 'processing', 'completed', 'preparing', 'shipped' ), true )) {
		...
		$low_stock_ids = class_exists( 'BSC_Stock' ) ? BSC_Stock::get_low_stock_products( $low_threshold ) : array();
	...
	$metric_counters     = array_merge( bsc_dashboard_empty_metrics()['counters'], is_array( $metrics['counters'] ?? null ) ? $metrics['counters'] : array() );
			...
			<div class="bsc-admin-dashboard__insight-panels">
				<div>
					...
					<table class="wp-list-table widefat striped">
						...
						<tbody>
						<?php foreach ( (array) ( $metrics['top_products'] ?? array() ) as $product_row ) : ?>
							<tr>
								<td>
				...
				<div>
					...
					<table class="wp-list-table widefat striped">
						...
						<tbody>
						<?php foreach ( (array) ( $metrics['no_result_searches'] ?? array() ) as $search_row ) : ?>
							<tr>
		...
		<div class="bsc-admin-dashboard__panels">
			...
			<div>
				...
				<table class="wp-list-table widefat striped">
					...
					if ( $can_view_financials ) :
						?>
						<th>Total</th><?php endif; ?><th>Estado</th></tr></thead>
					<tbody>
					...
					<?php endforeach; ?>
					<?php
					if (empty( $kpis['recent_orders'] )) :
						?>
						<tr><td colspan="<?php echo esc_attr( $can_view_financials ? 4 : 3 ); ?>" class="bsc-admin-dashboard__empty-row">Sin pedidos.</td></tr><?php endif; ?>
					</tbody>
				</table>
			...
			<div>
				...
				<table class="wp-list-table widefat striped bsc-admin-dashboard__low-stock-table">
					...
					<tbody>
					<?php
					foreach ($kpis['low_stock_ids'] as $pid) :
...
function bsc_render_settings_page(): void {
	...
	if ( 'POST' === $request_method && isset( $_POST['bsc_settings_nonce'] ) ) {
		if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['bsc_settings_nonce'] ) ), 'bsc_settings_action' ) ) {
			wp_die( esc_html__( 'Solicitud no válida.', 'bsc-2-0' ) );
		}

		$bsc_whatsapp_number = sanitize_text_field( wp_unslash( $_POST['bsc_whatsapp_number'] ?? '573156922859' ) );

		update_option( 'bsc_whatsapp_number', preg_replace( '/[^0-9]/', '', $bsc_whatsapp_number ) );
