...
function bsc_creators_get_applications(): array {
	...
	return is_array( $applications ) ? array_values( $applications ) : array();
}
	...
	if (isset( $_POST['bsc_creator_action'] )) {
		...
		if ('delete_application' === $action) {
			...
			$applications = function_exists( 'bsc_privacy_delete_indexed_row' )
				...
				: array_values( array_diff_key( bsc_creators_get_applications(), array( $index => true ) ) );
...
function bsc_creators_export_csv(): void {
	...
	header( 'Content-Disposition: attachment; filename="bubble-creators-' . wp_date( 'Y-m-d' ) . '.csv"' );
	header( 'Pragma: no-cache' );
	header( 'Expires: 0' );

	$out = fopen( 'php://output', 'w' );
	fwrite( $out, "\xEF\xBB\xBF" );
	fputcsv( $out, array( 'Fecha', 'Estado', 'Nombre', 'Email', 'Instagram', 'TikTok', 'Origen', 'Mensaje', 'Notas', 'Consentimiento', 'IP hash', 'Retencion dias' ) );
...
function bsc_render_creators_page(): void {
	...
	if (isset( $_GET['bsc_notice'] )) {
		$notice_message = 'deleted' === sanitize_key( wp_unslash( $_GET['bsc_notice'] ) ) ? 'Solicitud eliminada.' : 'Solicitud actualizada.';
	}
