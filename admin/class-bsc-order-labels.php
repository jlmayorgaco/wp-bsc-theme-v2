...
final class BSC_Order_Label_ViewModel {
    ...
    private static function resolve_contains( WC_Order $order ): string {
        ...
        if ( $count <= 3 ) {
            $short = array_map( [ __CLASS__, 'truncate_for_summary' ], $names );
...
function bsc_build_order_label_pdf_stream( BSC_Order_Label_ViewModel $vm, float $width_mm, float $height_mm ): string {
    ...
    $commands[] = bsc_label_pdf_rect( 0, 0, $width_mm, 35, $height_mm, [ 17, 17, 17 ] );
    $commands[] = bsc_label_pdf_text( ':)', 6, 10, 'F2', 24, $height_mm, [ 255, 255, 255 ] );
    $commands[] = bsc_label_pdf_text( 'delicado', 25, 11.5, 'F2', 22, $height_mm, [ 255, 255, 255 ] );
    $commands[] = bsc_label_pdf_text( 'NO poner peso encima :)', 25, 23.5, 'F1', 7.2, $height_mm, [ 255, 255, 255 ] );

    $commands[] = bsc_label_pdf_line( 0, 51, $width_mm, 51, $height_mm, 0.3, [ 204, 204, 204 ] );
    $commands[] = bsc_label_pdf_line( 22.33, 35, 22.33, 51, $height_mm, 0.25, [ 187, 187, 187 ], true );
    $commands[] = bsc_label_pdf_line( 44.66, 35, 44.66, 51, $height_mm, 0.25, [ 187, 187, 187 ], true );
    $commands[] = bsc_label_pdf_line( 67, 35, 67, 51, $height_mm, 0.25, [ 187, 187, 187 ], true );

    $commands[] = bsc_label_pdf_center_text( '100% Original', 0, 43.2, 22.33, 'F2', 6.2, $height_mm, [ 17, 17, 17 ] );
    $commands[] = bsc_label_pdf_center_text( 'K-beauty lover', 22.33, 43.2, 22.33, 'F2', 6.2, $height_mm, [ 17, 17, 17 ] );
    $commands[] = bsc_label_pdf_center_text( 'Glow gang', 44.66, 43.2, 22.34, 'F2', 6.2, $height_mm, [ 17, 17, 17 ] );
    $commands[] = bsc_label_pdf_text( 'CONTIENE', 70, 37.3, 'F2', 6.1, $height_mm, [ 85, 85, 85 ] );
    ...
    $current_top = 66.5;
    $commands[] = bsc_label_pdf_text( $vm->name, 5, $current_top, 'F2', 13.5, $height_mm, [ 17, 17, 17 ] );
    $current_top += 9.5;

    if ( $vm->phone !== '' ) {
        $commands[] = bsc_label_pdf_text( $vm->phone, 5, $current_top, 'F2', 10.8, $height_mm, [ 17, 17, 17 ] );
        $current_top += 7.2;
    }

    $address_lines = bsc_label_pdf_wrap_text( trim( $vm->address_line1 . ' ' . $vm->address_line2 ), 32, 3 );
    foreach ( $address_lines as $address_line ) {
        $commands[] = bsc_label_pdf_text( $address_line, 5, $current_top, 'F1', 9.4, $height_mm, [ 17, 17, 17 ] );
        $current_top += 5.8;
    }

    $location = trim( implode( ', ', array_filter( [ $vm->city, $vm->state ] ) ) );
    if ( $location !== '' ) {
        $location_lines = bsc_label_pdf_wrap_text( strtoupper( $location ), 28, 2 );
        foreach ( $location_lines as $location_line ) {
            $commands[] = bsc_label_pdf_text( $location_line, 5, $current_top, 'F2', 8.4, $height_mm, [ 17, 17, 17 ] );
            $current_top += 5.4;
        }
    }

    $commands[] = bsc_label_pdf_line( 0, 105, $width_mm, 105, $height_mm, 0.3, [ 204, 204, 204 ] );
    $commands[] = bsc_label_pdf_text( 'OBSERVACIONES', 5, 108, 'F2', 6.1, $height_mm, [ 85, 85, 85 ] );
    $commands[] = bsc_label_pdf_multiline_text(
    ...
    $commands[] = bsc_label_pdf_line( 0, 125, $width_mm, 125, $height_mm, 0.3, [ 204, 204, 204 ] );
    $commands[] = bsc_label_pdf_center_text( 'BSC', 0, 131.8, $width_mm, 'F2', 23, $height_mm, [ 17, 17, 17 ] );
    $commands[] = bsc_label_pdf_waves( 34, 139.6, 32, $height_mm );
    $commands[] = bsc_label_pdf_center_text( 'SKIN   CARE   FIRST', 0, 145.4, $width_mm, 'F2', 8.2, $height_mm, [ 17, 17, 17 ] );
...
function bsc_label_pdf_waves( float $x_mm, float $top_mm, float $width_mm, float $page_height_mm ): string {
    ...
    $commands   = [ 'q ' . bsc_label_pdf_color( [ 17, 17, 17 ] ) . ' RG 1.2 w [] 0 d' ];
